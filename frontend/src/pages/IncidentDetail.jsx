...
export default function IncidentDetail() {
  ...
  // 📄 PDF Export Function (FIXED FOR VITE)
  const exportToPDF = () => {
    const doc = new jsPDF();
    ...
    doc.text(`Status: ${data.status}`, 20, 75);
    
    // Root Cause
    doc.setFontSize(14);
    doc.setTextColor(37, 99, 235);
    doc.text('Root Cause Analysis', 20, 90);
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 0);
    doc.text(data.root_cause || 'Not identified', 20, 100);
    
    // Affected Systems
    doc.setFontSize(14);
    doc.setTextColor(37, 99, 235);
    doc.text('Affected Systems', 20, 115);
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 0);
    data.affected_systems.forEach((sys, index) => {
      doc.text(`- ${sys}`, 25, 125 + (index * 7));
    });
    
    // AI Explanation
    let yPos = 125 + (data.affected_systems.length * 7) + 15;
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Generated by Enterprise Digital Shadow | Page ${i} of ${pageCount}`,
        20,
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen font-sans">
      ...
      {/* Incident Header with Export Button */}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {data.affected_systems.map(sys => (
                  <span key={sys} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm font-medium">
                    {sys}
                  </span>
                ))}
              </div>
